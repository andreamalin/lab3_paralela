...
int Parallel_scalar_product(double local_x[], double local_y[],
      double local_z[], int local_n);
int Parallel_scalar_vector(double local_vector[], double scalar, int local_n);
void random_fill(double  rand_vector[], int n );
...
int main(void) {
   ...
   random_fill(local_x, local_n);
   random_fill(local_y, local_n);


   Parallel_vector_sum(local_x, local_y, local_z, local_n);
   

    int result = Parallel_scalar_product(local_x, local_y, local_z, local_n);
    int final_res;
    MPI_Reduce(&result, &final_res, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);

   Print_vector(local_x, 10, n, "x RANDOM -> The first 10 elements are: ", my_rank, comm);
   Print_vector(local_y, 10, n, "y RANDOM -> The first 10 elements are: ", my_rank, comm);
   Print_vector(local_z, 10, n,  "The first 10 elements are: ", my_rank, comm);

   printf("\n");
   Print_vector(local_x, 10, n, "x RANDOM -> The last 10 elements are: ", my_rank, comm);
   Print_vector(local_y, 10, n, "y RANDOM -> The last 10 elements are: ", my_rank, comm);
   Print_vector(local_z, 10, n,  "The last 10 elements are: ", my_rank, comm);


    Parallel_scalar_vector(local_x, 10, local_n);
    Parallel_scalar_vector(local_y, 10, local_n);
   printf("\n");
   Print_vector(local_x, 10, n, "x RANDOM SCALAR 10 -> The last 10 elements are: ", my_rank, comm);
   Print_vector(local_y, 10, n, "y RANDOM SCALAR 10 -> The last 10 elements are: ", my_rank, comm);
...
int Parallel_scalar_product(
   ...
   int scalar_result;
   MPI_Reduce(&local_result, &scalar_result, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
   return scalar_result;
}  /* Parallel_vector_sum */

int Parallel_scalar_vector(
